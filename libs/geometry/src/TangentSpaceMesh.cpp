...
void TangentSpaceMesh::getAux(AuxAttribute attribute, filament::math::float2* out, size_t stride) const noexcept {
    AttributeImpl attrib = static_cast<AttributeImpl>(attribute);
    auto inAux = mInput->aux<float2>(attrib);
    ASSERT_PRECONDITION(inAux, "Must provide input auxilliary attribute");
    stride = stride ? stride : sizeof(decltype(*out));
    auto const& outAux = mOutput->aux<float2>(attrib);
    for (size_t i = 0; i < mOutput->vertexCount; ++i) {
        *out = outAux[i];
        takeStride(out, stride);
    }
...
void TangentSpaceMesh::getAux(AuxAttribute attribute, filament::math::float3* out, size_t stride) const noexcept {
    AttributeImpl attrib = static_cast<AttributeImpl>(attribute);
    auto inAux = mInput->aux<float3>(attrib);
    ASSERT_PRECONDITION(inAux, "Must provide input auxilliary attribute");
    stride = stride ? stride : sizeof(decltype(*out));
    auto const& outAux = mOutput->aux<float3>(attrib);
    for (size_t i = 0; i < mOutput->vertexCount; ++i) {
        *out = outAux[i];
        takeStride(out, stride);
    }
...
void TangentSpaceMesh::getAux(AuxAttribute attribute, filament::math::float4* out, size_t stride) const noexcept {
    AttributeImpl attrib = static_cast<AttributeImpl>(attribute);
    auto inAux = mInput->aux<float4>(attrib);
    ASSERT_PRECONDITION(inAux, "Must provide input auxilliary attribute");
    stride = stride ? stride : sizeof(decltype(*out));
    auto const& outAux = mOutput->aux<float4>(attrib);
    for (size_t i = 0; i < mOutput->vertexCount; ++i) {
        *out = outAux[i];
        takeStride(out, stride);
    }
...
void TangentSpaceMesh::getAux(AuxAttribute attribute, filament::math::ushort3* out, size_t stride) const noexcept {
    AttributeImpl attrib = static_cast<AttributeImpl>(attribute);
    auto inAux = mInput->aux<ushort3>(attrib);
    ASSERT_PRECONDITION(inAux, "Must provide input auxilliary attribute");
    stride = stride ? stride : sizeof(decltype(*out));
    auto const& outAux = mOutput->aux<ushort3>(attrib);
    for (size_t i = 0; i < mOutput->vertexCount; ++i) {
        *out = outAux[i];
        takeStride(out, stride);
    }
...
void TangentSpaceMesh::getAux(AuxAttribute attribute, filament::math::ushort4* out, size_t stride) const noexcept {
    AttributeImpl attrib = static_cast<AttributeImpl>(attribute);
    auto inAux = mInput->aux<ushort4>(attrib);
    ASSERT_PRECONDITION(inAux, "Must provide input auxilliary attribute");
    stride = stride ? stride : sizeof(decltype(*out));
    auto const& outAux = mOutput->aux<ushort4>(attrib);
    for (size_t i = 0; i < mOutput->vertexCount; ++i) {
        *out = outAux[i];
        takeStride(out, stride);
    }
